...
int pds::getSource(cstring str, int dest, int& value)
{	
	char* p = str;
	bool bInvert = false;	
	int nInvertNum = 136; 	
	...
	//////////// S, SCn ///////////
		case 'S': // SCAN
		{
			...
			if (*(p + 1) == 'C')
			{
				...
				gERR.addErr(ERR_PDS, __FUNCTION__, "S -> SC Context", num);
			}
...
void pds::setPDS(int dest, const char* source)
{
	...
	int type = PDS_DR;
//	cstring sdebug;
	if (dest >= DR0 && dest <= DR43_B) {
		type = PDS_DR; 
//		sdebug.Format("ADR%d", (dest - ADDR0)/2 );
	}else if (dest >= IO0 && dest <= IO35_B){
		type = PDS_IO;
...
//	printf("PDS SET %s(%d) = %s\n", sdebug.c_str() ,dest,  source);
	while (1)
	{
		...
		if (cnt % 2 == 0) {  // Check Comma ','
			int ret = getSource(tok.s, type, setVal);
			if (ret == 0) {
				gERR.addErr(ERR_PDS, __FUNCTION__, "SET Source Context Error.");
				break;
			}
...
//			printf("setVal %s  %d\n", tok.s.c_str(), setVal);			
			...
			{ 
				...
				if (dest % 2 == 0) gIO.writePG(adr + 1, setVal, 1);	// SDR
				gIO.writePG(adr, setVal, 1);						// DDR				
			}
			cypindex++;
		}
		else
