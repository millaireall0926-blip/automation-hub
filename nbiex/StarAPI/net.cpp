...
{
	cstring str;
	str.Format("<power-ex><set>");
	str.AppendFormat("<dps>");
	for (int i = 0; i < MAX_PMON_CH; i++)
	{
		str.AppendFormat("%.2f", gPWR.m_PMonArr[i].sv);
		str.AppendFormat("%.2f", gPWR.m_PMonArr[i].oc);
	}
	str.AppendFormat("</dps>");

	str.AppendFormat("<hdps>");
	for (int i = 0; i < 4; i++) {
		str.AppendFormat("%.2f,", gPWR.m_PMonArr[MAX_DPS_CH + i].sv);
	}
	str.AppendFormat("</hdps>");
	
	str.AppendFormat("<dac>");
	for (int i = 0; i < 3; i++) {
		str.AppendFormat("%.2f,", gPWR.m_VMonArr[i]);
	}
	str.AppendFormat("</dac>");
	str.AppendFormat("</set></power-ex>");
...
int net::sendPower()
{
	...
	for (int i = 0; i < MAX_DPS_CH; i++)
	{
		...
		if (m_nSendPower == SEND_PWR_READ) {
			rv = gPWR.m_PMonArr[i].rv;
			rc = gPWR.m_PMonArr[i].rc;
		}
	...
	for (int i = 0; i < 4; i++) {
		...
		float rc = 0;
		if (m_nSendPower == SEND_PWR_READ) {
			rv = gPWR.m_PMonArr[MAX_DPS_CH + i].rv;
			rc = gPWR.m_PMonArr[MAX_DPS_CH + i].rc;
		}
