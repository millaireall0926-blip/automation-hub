...
void failmem::readFmComp(int scanIdx)
{
	...
	gIO.readPGData(adrH, m_fmCompHArr[scanIdx], COMP_MEM_SZ / 2);
	adrH = PG_SYSTEM | 296; // H sdr
	gIO.readPGData(adrH, &m_fmCompHArr[scanIdx][8], COMP_MEM_SZ / 2);


	adrL = PG_SYSTEM | 357; // L ddr
...
void failmem::showFmData(int ofs, int size, int Mode)
{
	int loopCnt = ofs + size;
	if (loopCnt > FM_DEPTH)return;
	FM_MEM* pfml;
	...
	for (int i = ofs; i < loopCnt; i++) {
		...
		if (Mode == SHOW_BOTH) {
			...
			for (int j = 0; j < 18; j += 2)
				printf("%04x ", pfmh->ioComp[j] | (pfmh->ioComp[j + 1] << 8));
		}
		else if (Mode == SHOW_L) {
			for (int j = 0; j < 18; j += 2)
				printf("%04x ", pfml->ioComp[j] | (pfml->ioComp[j + 1] << 8));
		}
		printf("\n");
