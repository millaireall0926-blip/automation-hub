...
uint dps::I2cSendByte(int i2c_num, int command)
{
	...
	byte ret = gIO.dps_send_byte(i2c_num, command);
	if (ret != RTN_OK) MESSAGE("I2cSendByte() EOS Fail", WARN);
...
{	
	...
	for(int nDev = 0; nDev < MAX_DPS_DEVICE; nDev++) {
		byte ret = gIO.dps_send_byte(nDev, command);
		if (ret != RTN_OK) MESSAGE("I2cSendByteAll() EOS Fail", WARN);
...
bool dps::I2cWaitEos()
{
	...
	if (ret != RTN_OK)	MESSAGE("I2cWaitEos() EOS Fail", WARN);
	return (ret == RTN_OK);
}
...
bool dps::I2cWrite(int i2c_num, int command, int data, LTC_PMBUS_COMMAND_TYPE type)
{
	...
	uint8_t rv = gIO.dps_write(i2c_num, command, iLen, (byte *)(&data));
	
	if(rv != RTN_OK) {
		MESSAGE("I2cWrite() EOS Fail", WARN);
...
uint dps::I2cRead(int i2c_num, int command, LTC_PMBUS_COMMAND_TYPE type)
{
	...
	byte ret = gIO.dps_read(i2c_num, command, uLen, (byte *)(&uData));
	if (ret != RTN_OK) MESSAGE("I2cRead() EOS Fail", WARN);
...
bool dps::I2cReadA(int i2c_num, int command, uint uLen, char *pData)
{
	...
	if (ret != RTN_OK) MESSAGE("I2cReadA() EOS Fail", WARN);
	return (ret == RTN_OK);
}
...
{
	return GetLtcCommandR( I2C_CMD_MFR_PADS, ch);
}
...
float dps::ReadPout( uint i2c_num)
{	

	return GetLtcCommandL( I2C_CMD_READ_POUT, i2c_num);
}

float dps::ReadPin(uint i2c_num)
{	
...
bool dps::AllChannelInit()
{
	...
	for (unsigned int num = 0; num < MAX_DPS_DEVICE; num++)
	{		
		int cnt = 0; 
		for (unsigned int idx = 0; idx < vecInitCmd.size(); idx++)
		{
			...
			while(nTry--) {
				if(I2cWrite( num, cmd, uData, (LTC_PMBUS_COMMAND_TYPE)type) == false)
					printe("init.err device %d , cmd %s data 0x%X\n", num, sCmd.c_str(), uData );
			}
...
bool dps::SetVout(int ch, float fVolt, float under, float over, float limit, bool bCalApply) /// 0(0,1), 1(2,3), 2(4), 3(5)
{
	...
	if(bCalApply)
		fVolt = (fVolt *  gPWR.m_fCalDpsF[ch][0]) + gPWR.m_fCalDpsF[ch][1];

	float flimit = limit;
	if(bCalApply) {
		//float flimit = (limit/m_IMCal[ch].gain) - m_IMCal[ch].offset;
		flimit = (limit * gPWR.m_fCalDpsA[ch][0]) + gPWR.m_fCalDpsA[ch][1];
	}

	if (!SetLtcPage(i2c_num, nPage))
		printe("==== Error(ch%d) : I2C_CMD_PAGE(0x%X) ====\n", ch, I2C_CMD_PAGE);

	uint rPage = GetLtcCommandR(I2C_CMD_PAGE, i2c_num);
	...
	if (!SetLtcCommandL( I2C_CMD_IOUT_OC_WARN_LIMIT, i2c_num, flimit))
...
{	
	...
	Volt = -1.0;
	SetLtcPage( i2c_num, ltc_page);
	Volt = ReadVout( DPS_GROUP_SINGLE::DPS_SINGLE_SEL, i2c_num, cnt);
	if(bCalApply)
		Volt = (Volt * gPWR.m_fCalDpsV[ch][0]) + gPWR.m_fCalDpsV[ch][1];
	return true;
}
...
float dps::ReadChIout(int ch, int cnt, bool bCalApply)   
{
	...
	SetLtcPage( i2c_num, ltc_page);
	fValue = ReadIout( DPS_GROUP_SINGLE::DPS_SINGLE_SEL, i2c_num, cnt);

	if(bCalApply) {
		return ((fValue * gPWR.m_fCalDpsA[ch][0]) + gPWR.m_fCalDpsA[ch][1]);
	}
	else
		return fValue;
}

uint dps::ReadStatus(int i2c_num)
{	
	return  GetLtcMfrPads( i2c_num);	
}
...
bool dps::ReadAllreg(int nDps)
{
	...
	for( int nPage = 0; nPage < 2; nPage++) {
		...
		SetLtcPage(nDps, nPage);
		printf("LTC REG READ device %d , page : %d\n" , nDps, nPage);
		int cnt = 0; 
		for (unsigned int idx = 0; idx < vecReadCmd.size(); idx++)
		{
			...
			uData = I2cRead(nDps, cmd, (LTC_PMBUS_COMMAND_TYPE)type);
