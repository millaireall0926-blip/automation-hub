...
#define COMP_MEM_SZ		18
#define FM_SCAN_SZ		256

#define MAX_IO_CNT		144
...
typedef struct {
	byte dutCum[2];		// 2byte
	byte blkAdr[3];     // 3byte
	byte dummy[3];		// 3byte (4에서 3으로 수정)
	byte ioDrv[2];      // 2byte
	byte adrDrv[5];     // 5byte
	byte scan;          // 1byte
	byte reserved[15];  // 15byte (16에서 15로 수정)
...
public:
	...
	void showFmData(int ofs, int size, int Mode = SHOW_BOTH);
