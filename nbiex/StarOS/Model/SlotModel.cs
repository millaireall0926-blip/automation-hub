...
namespace StarOS.Model {
    ...
    public class SlotModel : BaseVM
    {
        ...
        public void  clearPowerTalble()
        {
            ...
            {
                _HVs[i].SetV = string.Format("{0,5}", "-");
                _HVs[i].GetC = string.Format("{0,5}", "-");
                _HVs[i].GetV = string.Format("{0,5}", "-");
                _HVs[i].OC   = string.Format("{0,5}", "-");
			}

            for(i = 0; i < PowerVM.DAC_SIZE; i++)
            {
                _DACs[i].SetV = string.Format("{0,5}", "-");
			}
        }
        ...
        public HVModel[] _HVs = new HVModel[PowerVM.HV_SIZE];
        public DACModel[] _DACs = new DACModel[PowerVM.DAC_SIZE];
...
//            Global.gMainVM.updateUIFlag((int)UI.SLOTLOG);
        ...
        public void addPowerLogSet()
        {
            ...
            for(i = 0; i < PowerVM.HV_SIZE; i++)
            {
                data += string.Format($"\t{_HVs[i].SetV}\t{_HVs[i].OC}");
            }

            for(i = 0; i < PowerVM.DAC_SIZE; i++)
            {
                data += string.Format($"\t{_DACs[i].SetV}");
            }
        ...
        public void addPowerLog()
        {
            ...
            for(i = 0; i < PowerVM.HV_SIZE; i++)
            {
                data += string.Format($"\t{_HVs[i].GetV}\t{_HVs[i].GetC}");
            }
