    ...
    public class RackModel : BaseVM
    {
        ...
        */
        ...
        public void GetSlideOpen(bool isOpen)
        {
            ...
            {
                //if(!_islideopen)
                showAlram(ALRAM_TYPE.ERROR, Global.msg_SlideOpen, false);
            }
            else
            {
                if (_islideopen)
		...
		public DelegateCommand RackStatToggle { get; set; }
        ...
        public RackModel(int ch)
        {
            ...
            */
			...
			RackStatToggle = new DelegateCommand(_ => ToggleRackStat());
		}
		private void ToggleRackStat()
		{
			switch (RackStat)
			{
				case RACK_STAT.STOP:
					RackStat = RACK_STAT.PRERUN;
                    CBoxs.ForEach(bm => bm.CBoxStat = CBOX_STAT.PRERUN);
					break;
				case RACK_STAT.PRERUN:
					RackStat = RACK_STAT.RUN;
					CBoxs.ForEach(bm => bm.CBoxStat = CBOX_STAT.RUN);
					break;
				case RACK_STAT.RUN:
					RackStat = RACK_STAT.DEFROST;
					CBoxs.ForEach(bm => bm.CBoxStat = CBOX_STAT.DEFROST);
					break;
				case RACK_STAT.DEFROST:
					RackStat = RACK_STAT.DONE;
					CBoxs.ForEach(bm => bm.CBoxStat = CBOX_STAT.DONE);
					break;
				case RACK_STAT.DONE:
					RackStat = RACK_STAT.ERROR;
					CBoxs.ForEach(bm => bm.CBoxStat = CBOX_STAT.ERROR);
					break;
				case RACK_STAT.ERROR:
					RackStat = RACK_STAT.STOP;
					CBoxs.ForEach(bm => bm.CBoxStat = CBOX_STAT.STOP);
					break;
				default:
					RackStat = RACK_STAT.STOP;
					break;
