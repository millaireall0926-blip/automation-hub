...
//		printf("star_server::responseFile: fread fail\n");
	s->starResult(nCmdSeq, status, buf);
	delete buf;
	return S_ERR_NONE;
}
...
char* star_server::cmdProcess(star_session* s, char* str)
{
	...
	if (strcasecmp(cmd, "add") == 0) {  // �̹� pgm�� diag�� add �Ǿ� ����
		...
		clIO.writePG(adr, 2, 1);
		printf("pg stop adr %x = 2\n", adr);
		...
		//DPS OFF
		//clDps.SetVout(0, 0.0, -0.01, 0.01, 0.001);
		//clDps.SetVout(1, 0.0, -0.01, 0.01, 0.001);
		//clDps.SetVout(2, 0.0, -0.01, 0.01, 0.001);
		//clDps.SetVout(3, 0.0, -0.01, 0.01, 0.001);
		clIO.set_hv_dac(2, 0.0);
		clIO.dps_run(0xf, 0);
		clIO.dps_vs3f(0);
		printf("DPS Power Off\n");
		printf("VS3 Power Off\n");
...
uint star_server::readDPSRom(int sel, int type)
{
	...
	uint dps = 0;
	clIO.dps_read(i2c_num, adr, 2, (byte*)&dps);
	return dps;
}
...
void star_server::writeDPSRom(int sel, int type, word value)
{
	...
	clIO.dps_write(i2c_num, adr, 2, (byte*)&value);
	usleep(1000);
	uint nn = 0;
	clIO.dps_read(i2c_num, 0x15, 1, (byte*)&nn); // Store User data 		
	
}


int star_server::setBidRefVolt(double fVolt)
{
	double setVolt = fVolt;
	double wVolt = setVolt;
	int chNo = 5;
	uint16_t dacCode = 0;
	dacCode = (uint16_t)(wVolt / (DAC_SPAN / DAC_RESOLUTION)) & 0xFFF;
	printf("DAC.Set : ch[ %01X ] volt[ %6.3f ] decode[ %03X ]\n", chNo, setVolt, dacCode);
	return clIO.dac_write(DAC_C_WO_SINREG, 5, dacCode);
}
...
int star_server::rwCal(uint16_t command, int32_t length, uint8_t* data, FILE* pf)
{
	uint16_t addr = command;
	uint32_t size = length;
	
	for (int i = 0; i < 5; i++) {
		int rv = clRom.read_eeprom(BUSID_EC_I2C, TCB_ROM, addr, size, data);
		if (rv != 0) {
			printf("eeprom Read Error Retry %d\n", i);
			fclose(pf);
			remove("/star/diag/cal/pecal.data");
			if (i >= 4)
				return 0;
		}
		else
...
int star_server::rwCal2(uint16_t command, int32_t length, uint8_t* data, FILE* pf)
{
	uint16_t addr = command;
	uint32_t size = length;
	...
	for (int i = 0; i < 5; i++) {
		...
		if (rv != 0) {
			printf("eeprom Read Error Retry %d\n", i);
			fclose(pf);
			remove("/star/diag/cal/dpscal.data");
			if (i >= 4)
				return 0;
		}
		else
...
int star_server::rwCal3(uint16_t command, int32_t length, uint8_t* data, FILE* pf)
{
	uint16_t addr = command;
	uint32_t size = length;
	...
	for (int i = 0; i < 5; i++) {
		...
		if (rv != 0) {
			printf("eeprom Read Error Retry %d\n", i);
			fclose(pf);
			remove("/star/diag/cal/vs3cal.data");
			if (i >= 4)
				return 0;
		}
		else
...
void star_server::makeCalFile()
{
	FILE* fpCal = fopen("/star/diag/cal/pecal.data", "wb");
	if (fpCal == NULL) {
		printf("Cal data file open fail.\n");
		return;
	}
	printf("MAKE CalData /star/diag/cal/pecal.data\n");
	float calBuf[256];	
	int sz = 0x20;
	...
	// ADC0 Gain
	if (rwCal(CAL_ADC0_ADDR, sz, (uint8_t*)calBuf, fpCal) == 0) return;
	// ADC0 Offset
	if (rwCal(CAL_ADC0_ADDR + sz, sz, (uint8_t*)calBuf, fpCal) == 0) return;

	// ADC1 Gain
	if (rwCal(CAL_ADC1_ADDR, sz, (uint8_t*)calBuf, fpCal) == 0) return;
	// ADC1 Offset
	if (rwCal(CAL_ADC1_ADDR + sz, sz, (uint8_t*)calBuf, fpCal) == 0) return;

	
	// VIL Gain
	sz = sizeof(float) * 176;
	if (rwCal(CAL_PE_VIL_ADDR , sz, (uint8_t*)calBuf, fpCal) == 0) return;
	// VIL Offset
	if (rwCal(CAL_PE_VIL_ADDR + sz, sz, (uint8_t*)calBuf, fpCal) == 0) return;
	

	// VIH Gain
	sz = sizeof(float) * 176;
	if (rwCal(CAL_PE_VIH_ADDR, sz, (uint8_t*)calBuf, fpCal) == 0) return;	
	// VIH Offset
	if (rwCal(CAL_PE_VIH_ADDR + sz, sz, (uint8_t*)calBuf, fpCal) == 0) return;
	
	fclose(fpCal);
	printf("CLOSE CalData /star/diag/cal/pecal.data\n");
}

void star_server::makeCalFile2()
{
	FILE* fpCal = fopen("/star/diag/cal/dpscal.data", "wb");
	if (fpCal == NULL) {
		printf("Cal data file open fail.\n");
		return;
	}
	printf("MAKE CalData /star/diag/cal/dpscal.data\n");
	float calBuf[256];	
	int sz = 0x10;
	...
	// DPS VS Gain
	if (rwCal2(addr, sz, (uint8_t*)calBuf, fpCal) == 0) return;
	addr += sz;
	if(calBuf[0] < 0.9 || calBuf[0] > 1.1 || calBuf[1] < 0.9 || calBuf[1] > 1.1 ) {
		fclose(fpCal);
		remove("/star/diag/cal/dpscal.data");
		return;
	}
	...
	fclose(fpCal);
	printf("CLOSE CalData /star/diag/cal/dpscal.data\n");
}

void star_server::makeCalFile3()
{
	FILE* fpCal = fopen("/star/diag/cal/vs3cal.data", "wb");
	if (fpCal == NULL) {
		printf("Cal data file open fail.\n");
		return;
	}
	printf("MAKE CalData /star/diag/cal/vs3cal.data\n");
	float calBuf[256];	
	int sz = 0x10;

	usleep(1000*1000);

	uint16_t addr = CAL_VS3_ADDR;

	// VS3 Gain
	if (rwCal3(addr, sz, (uint8_t*)calBuf, fpCal) == 0) return;
	addr += sz;
	if(calBuf[0] < 0.85 || calBuf[0] > 1.1) {
		fclose(fpCal);
		remove("/star/diag/cal/vs3cal.data");
		return;
	}
	//VS3 Offset
	//if (rwCal3(addr, sz, (uint8_t*)&calBuf[1], fpCal) == 0) return;
	//addr += sz;
	//// DPS VS Offset 2
	//if (rwCal3(addr, sz, (uint8_t*)calBuf, fpCal) == 0) return;
	//addr += sz;
	...
	fclose(fpCal);
	printf("CLOSE CalData /star/diag/cal/vs3cal.data\n");

	printf("VS3 Gain: %0.5f, Offset: %0.5f\n", calBuf[0], calBuf[1]);
}
...
int star_server::setup(char *config_fn, char *dID, char* id, char* file)
{
	...
	// IO / FPGA Check
	{
		...
		if (m_bIO) {
			...
			{
				char id[64];
				strcpy(id, lib->xml_att_find(fh, "id"));
				char* p, * q;
				p = id;
				q = p + strlen(id);
				fpgaID = strtoul(p, &q, 16);
				printf("config FPGA : s:%s, d:%x\n", id, fpgaID);
			}

			int ret = clIO.cfg_fpga( "/star/test/pg_top.bit");
			if (ret != RTN_OK)
				fprintf(fpLog, "FPGA Upload Fail!\n");
			else {
				fprintf(fpLog, "FPGA Upload Success\n");
				if  (!clIO.checkPG(fpgaID)) fprintf(fpLog, "FPGA ID Read Fail!!\n");
			}
		}
		fclose(fpLog);
	...
	printf("=========================================\n");
	if (atoi(lib->xml_att_find(daemon_h, "calData"))) {
		if (m_bIO) {
			printf("0.=========================================\n");
			makeCalFile();
			printf("1.=========================================\n");
		...
		printf("Does not use calibration data.\n");
