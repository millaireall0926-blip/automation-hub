...
//                if (_isPgm != prev) _mvm.UpdateFlag_CMList = true;
        ...
        public void processPacket(CMPacket packetData) {
            ...
            string bodyLog = "";
            if (packetData.Body != null && packetData.Body.Length > 0)
            {
                try
                {
                    // Substring(자르기) 로직 제거됨 -> 전체 변환
                    bodyLog = System.Text.Encoding.Default.GetString(packetData.Body);
                }
                catch
                {
                    bodyLog = "[Binary Data Decode Error]";
                }
            }
            else
            {
            ...
            Console.WriteLine($"[RECV][CM:{Name}] Mode:0x{packetData.header.Mode:X4} Len:{packetData.header.Len}");
            // 내용이 64KB든 뭐든 전부 다 찍습니다.
            Console.WriteLine($"└─> Body: {bodyLog}");
