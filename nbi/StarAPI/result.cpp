...
void result::initResult()
{
	...
	XAS(hInfo, "pgm", gAPI.m_sPgm);
	XAS(hInfo, "lot", gAPI.m_sLot);
	XAS(hInfo, "bid", gAPI.m_sBid);
...
void result::loadResult()
{
	...
	sprintf(gAPI.m_sLot, gLIB.xml_att_find(h, "lot"));
...
void result::resultAddStepList()
{
	...
	{
		str.AppendFormat("%s", gAPI.m_sStepName[i]);
		if (i <= gAPI.m_nStepIdx - 1)
			str.AppendFormat(",");
	}
...
int result::startStepRes()
{
	...
	cstring str;
	str.Format("/test-result/step.id=%s", gAPI.m_sCurStep);
	HND hStep = gLIB.xml_path_handle(m_hResult, str.c_str());
	strcpy(m_sResCurStep, gAPI.m_sCurStep);
	...
	int con = 0;
	if (gAPI.m_nTestMode == TEST_MODE_CONTACT)con = 1;
	XAI(hStep, "con", con );
...
int result::endStepRes()
{
	...
	for (i = 0; i < gBIB.m_nDutCnt; i++)
	{
		if (i != 0 && (i % 32 == 0)) {
			...
			nTotal = 0;
			sMask.AppendFormat("0x%08x,", nMask);
			nMask = 0;
		...
		if (gBIB.getDut(i)->m_nTotalResult)
			nTotal |= 1 << (i % 32);		
		if (gBIB.getDut(i)->m_nDutMask)
			nMask |= 1 << (i % 32);
		

//		printf("dno[%d] cur %d tot %d cum0x%08x mask %d\n", gBIB.getDut(i)->m_nDutNo,
//			gBIB.getDut(i)->m_nCurResult, gBIB.getDut(i)->m_nTotalResult, nTotal, gBIB.getDut(i)->m_nDutMask);
	}	
	...
	for (int i = 0; i < gBIB.m_nDutCnt; i++)
	{
		if (gAPI.m_nTestMode == TEST_MODE_HANDLER)
			...
			if (gAPI.m_maskDuts[ i/ mc + i % mc])
				sBin.AppendFormat("%d,", 31);
			else if (gBIB.getDut(i)->m_nConResult == 0)
			{
				sBin.AppendFormat("%d,", 21);
			}
			else
			{
				sBin.AppendFormat("%x,", gBIB.getDut(i)->m_nResultBin);
			}
		}
	}
...
void result::saveResOption()
{
	...
	if (gBIB.m_nResultMode & RES_FAIL_BIT) {		
		...
		for (i = 0; i < gBIB.m_nDutCnt; i++)
		{
			...
			{
				if ((gAPI.m_nTestMode == TEST_MODE_CONTACT) || pDut->m_nConResult == STEP_PASS)
				{
	...
	if (gBIB.m_nResultMode & RES_MEAS_DATA_READ)
	{
		HND fh = gLIB.xml_create("sub");
		XAS(fh, "id", m_sResCurStep);
		XAS(fh, "lot", gAPI.m_sLot);
		XAS(fh, "type", "meas-data");
		cstring str = ""; str.Format("");
		for (int i = 0; i < gBIB.m_nDutCnt; i++)
		{
