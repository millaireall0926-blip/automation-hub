...
public:
	...
	bool SetLtcCommandLGlobal(DPS_PMBUS_COMMAND, int, float); // L11, L16
...
public:
	...
	bool I2cWriteGlobal(int, int, int, LTC_PMBUS_COMMAND_TYPE);
...
public:
	...
	float ReadPout(DPS_GROUP_SINGLE, uint);
	float ReadPin(DPS_GROUP_SINGLE, uint);
...
public:
	bool AllChannelInit();
	bool SetVout(int ch, float fVolt, float under, float over, float limit);
	bool SetHVout(float fVolt = -15.0);
	bool SetHVoutZero();
	bool ReadChVout(int ch, float &VMin, float &VMax);
	float ReadChIout(int ch, bool bCalApply = true);
	uint ReadAlertCtrl();
	float ReadCvmVout();
	float ReadCvmIout();
