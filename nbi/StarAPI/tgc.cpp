...
void tgc::initDelay()
{
	...
	writeClkDelay(0, 30);	//RATE	

	writeClkDelay(1, 0);	//CPEH
	writeClkDelay(2, 10);	//CPEL
		 
	writeClkDelay(5, 54);	//IO
	writeClkDelay(6, 0);	//ID_AD
	writeClkDelay(7, 54);	//DRE
	writeClkDelay(8, 35);	//ADDR
		 
	writeClkDelay( 9, 38 ); //CLK0
	writeClkDelay(10, 38 ); //CLK1
	writeClkDelay(11, 36 ); //CLK2
	writeClkDelay(12, 37 ); //CLK3
	writeClkDelay(13, 38 ); //CLK4
	writeClkDelay(14, 38 ); //CLK5
	writeClkDelay(15, 39 ); //CLK6
	writeClkDelay(16, 50 ); //CLK7
	writeClkDelay(17, 48 ); //CLK8
	writeClkDelay(18, 51 ); //CLK9
	writeClkDelay(19, 49 ); //CLK10
	writeClkDelay(20, 52 ); //CLK11
	writeClkDelay(21, 53 ); //CLK12
	writeClkDelay(22, 52 ); //CLK13
	writeClkDelay(23, 51 ); //CLK14
	writeClkDelay(24, 45 ); //CLK15
		 
	writeClkDelay(25, 4 );  //SCAN0
	writeClkDelay(26, 5 );  //SCAN1
	writeClkDelay(27, 0 );  //SCAN2
	writeClkDelay(28, 8 );  //SCAN3
}
...
void tgc::writeClkDelay(int select, int delay)
{	
	gIO.writePG(PG_SYSTEM | 64, delay, 1);
	gIO.writePG(PG_SYSTEM | 65, select, 1);
	usleep(1000 * 100);

	gIO.writePG(PG_SYSTEM | 66, 1, 1);
	gIO.writePG(PG_SYSTEM | 66, 0, 1);
}