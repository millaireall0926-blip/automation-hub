...
dps::dps()
{
	...
	m_VOutVal.assign(4, 0.0);
	m_UVFVal.assign(4, 0.0);
	m_OVFVal.assign(4, 5.0);
...
void dps::MakeCalFactor() {
	...
	for (int idx = 0; idx < MAX_DPS_USER_NUM; idx++) {
...
bool dps::GetDpsCommandInfo(DPS_PMBUS_COMMAND command, int &format, int &type, int &defaullt, string &sCmd)
{
	...
	{
		printf("%s : error DPS I2C COMMAND 0x%X\n", __FUNCTION__, command);
		return false;
	}
...
uint dps::I2cSendByte(int i2c_num, int command)
{
	//printf("Fault clear Start (i2cNo : %d)\n", i2c_num);
	gIO.dps_send_byte(i2c_num, command);
	printf("Fault clear Done (i2cNo : %d)\n", i2c_num);
	usleep(1000);
	//if (!I2cWaitEos())
	//{
...
{
	gIO.dps_send_byte(0, command);
	gIO.dps_send_byte(2, command);
	gIO.dps_send_byte(4, command);
	gIO.dps_send_byte(8, command);
	printf("All dps Fault clear Done \n");
	usleep(1000);
...
bool dps::I2cWaitEos()
{
	...
	if (gIO.m_bSim)
		return true;
	byte ret = gIO.dps_wait_eos();
	return (ret == RTN_OK);
}

// bool dps::I2cWrite( DPS_GROUP_SINGLE sel, int i2c_num, int command, int data, LTC_PMBUS_COMMAND_TYPE type )
bool dps::I2cWrite(int i2c_num, int command, int data, LTC_PMBUS_COMMAND_TYPE type)
{
	if (m_nDebug > 0)
	{
	...
	gIO.dps_write(i2c_num, command, iLen, (byte *)(&data));
...
bool dps::I2cWriteGlobal(int i2c_num, int command, int data, LTC_PMBUS_COMMAND_TYPE type)
{
	if (m_nDebug > 0)
	{
		printf("%s i2c(%d)\tcmd:%x\t%x\n", __FUNCTION__, i2c_num, command, data);
	...
	gIO.dps_write_global(i2c_num, command, iLen, (byte *)(&data));
...
uint dps::I2cRead(int i2c_num, int command, LTC_PMBUS_COMMAND_TYPE type)
{
	// printf("%s sim%d\n", __FUNCTION__, m_bSim);
	...
	gIO.dps_read(i2c_num, command, uLen, (byte *)(&uData));
...
bool dps::I2cReadA(int i2c_num, int command, uint uLen, char *pData)
{

	byte ret = gIO.dps_read(i2c_num, command, uLen, (byte *)(&pData));
...
bool dps::SetLtcCommandL(DPS_PMBUS_COMMAND command, int i2c_num, float fValue)
{
	cstring sCmd = "";
	uint16_t uData = 0x0;
	...
	{
		printf("%s : error I2C Command:0x%X!!\n", __FUNCTION__, (int)command);
		return false;
	}
	switch (format)
	{
	case FORMAT_L11:
		...
		// printf("%s : L11 [%5.3f][0x%04X]\n", sCmd.c_str(), fValue, uData );
		break;
	case FORMAT_L16:
		uData = FloatToL16(fValue);
		// printf("%s : L16 [%5.3f][0x%04X]\n", sCmd.c_str(), fValue, uData );
		break;
	default:
	...
	return I2cWrite(i2c_num, command, uData, (LTC_PMBUS_COMMAND_TYPE)type);
}

bool dps::SetLtcCommandLGlobal(DPS_PMBUS_COMMAND command, int i2c_num, float fValue)
{
	cstring sCmd = "";
	uint16_t uData = 0x0;
	int format, type, value;
	...
	// return I2cWrite( sel, i2c_num, command, uData, (LTC_PMBUS_COMMAND_TYPE)type );
	return I2cWriteGlobal(i2c_num, command, uData, (LTC_PMBUS_COMMAND_TYPE)type);
}

// REG
// bool dps::SetLtcCommandR( DPS_PMBUS_COMMAND command, DPS_GROUP_SINGLE sel, int i2c_num, uint uValue)
bool dps::SetLtcCommandR(DPS_PMBUS_COMMAND command, int i2c_num, uint uValue)
{
	cstring sCmd = "";
	uint16_t uData = 0x0;
	int format, type, value;
	...
	case FORMAT_NONE:
		...
		break;
	default:
		printf("%s : error invalid i2c data format expect REG or I16\n", sCmd.c_str());
		return false;
		break;
	}
	// return I2cWrite( sel, i2c_num, command, uData, (LTC_PMBUS_COMMAND_TYPE)type );
...
float dps::GetLtcCommandL(DPS_PMBUS_COMMAND command, int i2c_num) // i2c command, ch
{
	cstring sCmd = "";
	uint16_t uData = 0x0;
	float fData = 0.0;
	...
	uData = I2cRead(i2c_num, (uint)command, (LTC_PMBUS_COMMAND_TYPE)type);
	switch (format)
	{
	case FORMAT_L11:
...
uint dps::GetLtcCommandR(DPS_PMBUS_COMMAND command, int i2c_num) // i2c command, ch
{
	cstring sCmd = "";
	uint16_t uData = 0x0;
	int format, type, value;
...
bool dps::GetLtcCommandA(DPS_PMBUS_COMMAND command, int i2c_num, char *pDest) // ASC
{
	cstring sCmd = "";
	int format, type, len;
	...
	return I2cReadA(i2c_num, (uint)command, len, pDest);
}
...
bool dps::isValidGrp(uint i2c_grp)
{
	...
	{
		printf("%s : error Dps I2C_GROUP Set Error Expect 0 or 1, but %d\n", __FUNCTION__, i2c_grp);
		return false;
	}
	return true;
}
...
bool dps::isValidPage(uint page)
{
	...
	if (page < 0 || page > 1)
	{
		printf("%s : error Dps I2C_PAGE Set Error Expect 0 or 1, but %d\n", __FUNCTION__, page);
		return false;
	}
	return true;
}
...
bool dps::isValidVout(float fVolt)
{
	...
	{
		printf("%s : error Dps VOUT force value between [ %f ~ %f], but %f set\n", __FUNCTION__, m_fLtcVoutMin, m_fLtcVoutMax, fVolt);
		return false;
	}
	return true;
}
...
// VOUT MODE READ
uint dps::GetDpsVoutMode(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
		return false;
	}

	return GetLtcCommandR(I2C_CMD_VOUT_MODE, i2c_num);
}

// PAGE0 or PAGE1 or ALL
bool dps::SetLtcPage(uint i2c_num, uint page)
{
	if (!isValidPage(page))
	{
		printf("%s : error LTC%02d I2C Page use 0x0/0x1/0xFF, but %d\n", __FUNCTION__, i2c_num, page);
		return false;
	}
	return SetLtcCommandR(I2C_CMD_PAGE, i2c_num, page);
}

// ON, OFF, MARGIN HIGH/LOW
bool dps::SetLtcOperation(uint i2c_num, LTC_PMBUS_OPPERATION opr)
{
	return SetLtcCommandR(I2C_CMD_OPERATION, i2c_num, (uint)opr);
}

// ON_OFF_CONFIG
bool dps::SetLtcOnOffConfig(uint i2c_num, LTC_PMBUS_ON_OFF_CONFIG cfg)
{
	return SetLtcCommandR(I2C_CMD_ON_OFF_CONFIG, i2c_num, (uint)cfg);
}

// clear Faults
bool dps::SetLtcClearFaults(uint i2c_num, uint uData)
{
	return SetLtcCommandR(I2C_CMD_CLEAR_FAULTS, i2c_num, uData);
}

// clear Faults
bool dps::SetLtcMfrClearPeaks(uint i2c_num, uint uData)
{
	return SetLtcCommandR(I2C_CMD_MFR_CLEAR_PEAKS, i2c_num, uData);
}

// RESET
bool dps::SetLtcMfrReset(uint i2c_num, uint uData)
{
	bool bRtn = false;
	bRtn = SetLtcCommandR(I2C_CMD_MFR_RESET, i2c_num, uData);
	usleep(1);
	return bRtn;
}

bool dps::SetLtcStoreUserAll(uint i2c_num, uint uData)
{
	bool bRtn = false;
	bRtn = SetLtcCommandR(I2C_CMD_STORE_USER_ALL, i2c_num, uData);
	usleep(1);
	return bRtn;
}

bool dps::SetLtcRestoreUserAll(uint i2c_num, uint uData)
{
	bool bRtn = false;
	bRtn = SetLtcCommandR(I2C_CMD_RESTORE_USER_ALL, i2c_num, uData);
	usleep(1);
	return bRtn;
}

bool dps::setLtcMFRFaultPropagate(uint i2c_num, uint uData)
{
	bool bRtn = false;
	bRtn = SetLtcCommandR(I2C_CMD_MFR_FAULT_PROPAGATE, i2c_num, uData);
	usleep(1);
	return bRtn;
}

// Force Voltage Set
bool dps::SetLtcVoutCommand(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_COMMAND, i2c_num, fVolt);
}

// Upper limit Vout command output voltage
bool dps::SetLtcVoutMax(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_MAX, i2c_num, fVolt);
}

// Upper limit Vout command output voltage
bool dps::SetLtcVoutMarginHigh(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_MARGIN_HIGH, i2c_num, fVolt);
}

// Upper limit Vout command output voltage
bool dps::SetLtcVoutMarginLow(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_MARGIN_LOW, i2c_num, fVolt);
}

bool dps::SetLtcVoutTransRate(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_TRANSITION_RATE, i2c_num, fVolt);
}

// Freq KHZ
bool dps::SetLtcFreqSwitch(uint i2c_num, uint freq)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_FREQUENCY_SWITCH, i2c_num, freq);
}

bool dps::SetLtcVinOn(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VIN_ON, i2c_num, fVolt);
}

bool dps::SetLtcVinOff(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VIN_OFF, i2c_num, fVolt);
}

bool dps::SetLtcIOutCalGain(uint i2c_num, float fmOhm) // mili ohm
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_IOUT_CAL_GAIN, i2c_num, fmOhm);
}

bool dps::SetLtcVoutOvFaultLimit(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, i2c_num, fVolt);
}

bool dps::SetLtcVoutOvFaultResponse(uint i2c_num, uint uRes)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_VOUT_OV_FAULT_RESPONSE, i2c_num, uRes & 0xFFFF);
}

bool dps::SetLtcVoutOvWarnLimit(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_OV_WARN_LIMIT, i2c_num, fVolt);
}

bool dps::SetLtcVoutUvWarnLimit(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_UV_WARN_LIMIT, i2c_num, fVolt);
}

bool dps::SetLtcVoutUvFaultLimit(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VOUT_UV_FAULT_LIMIT, i2c_num, fVolt);
}

bool dps::SetLtcVoutUvFaultResponse(uint i2c_num, uint uRes)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_VOUT_UV_FAULT_RESPONSE, i2c_num, uRes & 0xFFFF);
}

bool dps::SetLtcIoutOcFaultLimit(uint i2c_num, float fCurr)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_IOUT_OC_FAULT_LIMIT, i2c_num, fCurr);
}

bool dps::SetLtcIoutOcFaultResponse(uint i2c_num, uint uRes)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_IOUT_OC_FAULT_RESPONSE, i2c_num, uRes & 0xFFFF);
}

bool dps::SetLtcIoutOcWarnLimit(uint i2c_num, float fCurr)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_IOUT_OC_WARN_LIMIT, i2c_num, fCurr);
}

bool dps::SetLtcOtFaultLimit(uint i2c_num, float fTemp)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_OT_FAULT_LIMIT, i2c_num, fTemp);
}

bool dps::SetLtcOtFaultResponse(uint i2c_num, uint uRes)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_OT_FAULT_RESPONSE, i2c_num, uRes);
}

bool dps::SetLtcOtWarnLimit(uint i2c_num, float fTemp)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_OT_WARN_LIMIT, i2c_num, fTemp);
}

bool dps::SetLtcUtFaultLimit(uint i2c_num, float fTemp)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_UT_FAULT_LIMIT, i2c_num, fTemp);
}

bool dps::SetLtcUtFaultResponse(uint i2c_num, uint uRes)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_UT_FAULT_RESPONSE, i2c_num, uRes);
}

bool dps::SetLtcVinOvFaultLimit(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VIN_OV_FAULT_LIMIT, i2c_num, fVolt);
}

bool dps::SetLtcVinOvFaultResponse(uint i2c_num, uint uRes)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_VIN_OV_FAULT_RESPONSE, i2c_num, uRes & 0xFFFF);
}

bool dps::SetLtcVinUvWarnLimit(uint i2c_num, float fVolt)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_VIN_UV_WARN_LIMIT, i2c_num, fVolt);
}

bool dps::SetLtcIinOcWarnLimit(uint i2c_num, float fCurr)
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_IIN_OC_WARN_LIMIT, i2c_num, fCurr);
}

bool dps::SetLtcTonDelay(uint i2c_num, float fTime) // ms
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_TON_DELAY, i2c_num, fTime);
}

bool dps::SetLtcTonRise(uint i2c_num, float fTime) // ms
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_TON_RISE, i2c_num, fTime);
}

bool dps::SetLtcTonMaxFaultLimit(uint i2c_num, float fTime) // ms
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_TON_MAX_FAULT_LIMIT, i2c_num, fTime);
}

bool dps::SetLtcToffDelay(uint i2c_num, float fTime) // ms
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_TOFF_DELAY, i2c_num, fTime);
}

bool dps::SetLtcToffFall(uint i2c_num, float fTime) // ms
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_TOFF_FALL, i2c_num, fTime);
}

bool dps::SetLtcToffMaxWarnLimit(uint i2c_num, float fTime) // ms
{
	// if( !isValidVout(fVolt) ) return false;
	// return SetLtcCommandL( I2C_CMD_TON_MAX_FAULT_LIMIT, sel, i2c_num, fTime );
	return SetLtcCommandL(I2C_CMD_TOFF_MAX_WARN_LIMIT, i2c_num, fTime);
}

bool dps::setMfrIinCalGain(uint i2c_num, float fGain)
{
	return SetLtcCommandL(I2C_CMD_MFR_IIN_CAL_GAIN, i2c_num, fGain);
}

bool dps::SetLtcMfrRVin(uint i2c_num, float fTime) // ohm
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandL(I2C_CMD_MFR_RVIN, i2c_num, fTime);
}

bool dps::SetLtcMfrPwmComp(uint i2c_num, uint pwm_comp) // REG
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_MFR_PWM_COMP, i2c_num, pwm_comp);
}

bool dps::SetLtcMfrPwmMode(uint i2c_num, uint pwm_mode) // REG
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_MFR_PWM_MODE, i2c_num, pwm_mode);
}

uint dps::getLtcMfrPwmMode(uint i2c_num)
{
	return GetLtcCommandR(I2C_CMD_MFR_PWM_MODE, i2c_num);
}

bool dps::SetLtcRailAddress(uint i2c_num, uint nRail) // REG
{
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_MFR_RAIL_ADDRESS, i2c_num, nRail);
}

bool dps::SetLtcPolyMode(uint i2c_num, uint nEnable) // REG
{
	uint8_t nData = 0x10;
	if (nEnable)
		nData |= 0x80;
	// if( !isValidVout(fVolt) ) return false;
	return SetLtcCommandR(I2C_CMD_MFR_PWM_CONFIG, i2c_num, nData);
}

// STATUS BYTE
uint dps::GetLtcStatusByte(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_BYTE, i2c_num);
}

uint dps::GetLtcStatusWord(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_WORD, i2c_num);
}

uint dps::GetLtcStatusVout(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_VOUT, i2c_num);
}

uint dps::GetLtcStatusIout(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_IOUT, i2c_num);
}

uint dps::GetLtcStatusInput(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_INPUT, i2c_num);
}

uint dps::GetLtcStatusTemperature(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_TEMPERATURE, i2c_num);
}

uint dps::GetLtcStatusCml(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_CML, i2c_num);
}

uint dps::GetLtcStatusMfrSpecific(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_STATUS_MFR_SPECEFIC, i2c_num);
}

uint dps::GetLtcMfrCommon(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_MFR_COMMON, i2c_num);
}
...
uint dps::GetLtcMfrPads(uint ch)
{
	uint i2c_num[4] = {0, 2, 4, 5};
	return GetLtcCommandR(I2C_CMD_MFR_PADS, i2c_num[ch]);
}

float dps::GetMfrVoutMax(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_VOUT_MAX, i2c_num);
}

float dps::ReadVin(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_READ_VIN, i2c_num);
}

float dps::ReadIin(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_READ_IIN, i2c_num);
}

float dps::ReadVout(DPS_GROUP_SINGLE sel, uint i2c_num, uint sampleing)
{
	float fMeas = 0.0;
	float fMeasSum = 0.0;
	if (sel == DPS_GROUP_SEL)
	...
	{
		fMeasSum += GetLtcCommandL(I2C_CMD_READ_VOUT, i2c_num);
	}
	fMeas = fMeasSum / (float)sampleing;
	return fMeas;
}

float dps::ReadIout(DPS_GROUP_SINGLE sel, uint i2c_num, uint sampleing)
{
	float fMeas = 0.0;
	float fMeasSum = 0.0;
	if (sel == DPS_GROUP_SEL)
	...
	{
		float fTemp = 0.0;
		fTemp = GetLtcCommandL(I2C_CMD_READ_IOUT, i2c_num);
...
float dps::ReadTemperature_1(DPS_GROUP_SINGLE sel, uint i2c_num)
{
...
//	usleep(8000);
	...
	return GetLtcCommandL(I2C_CMD_READ_TEMP_1, i2c_num);

}

float dps::ReadTemperature_2(DPS_GROUP_SINGLE sel, uint i2c_num)
{
...
//	usleep(8000);
	...
	return GetLtcCommandL(I2C_CMD_READ_TEMP_2, i2c_num);

}

float dps::ReadMfrTemperature_1_Peak(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_TEMPERATUR_1_PEAK, i2c_num);
}

float dps::ReadMfrTemperature_2_Peak(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_TEMPERATUR_2_PEAK, i2c_num);
}

float dps::ReadFrequency(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_READ_FREQUENCY, i2c_num);
}

float dps::ReadPout(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_READ_POUT, i2c_num);
}

float dps::ReadPin(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_READ_PIN, i2c_num);
}

float dps::ReadMfrVoutPeak(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_VOUT_PEAK, i2c_num);
}

float dps::ReadMfrIoutPeak(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_IOUT_PEAK, i2c_num);
}

float dps::ReadMfrIinPeak(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_READ_IIN_PEAK, i2c_num);
}

float dps::ReadMfrReadIChip(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_READ_ICHIP, i2c_num);
}

float dps::ReadMfrVinPeak(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_VIN_PEAK, i2c_num);
}

float dps::ReadMfrIinCalGain(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandL(I2C_CMD_MFR_IIN_CAL_GAIN, i2c_num);
}

uint dps::ReadMfrAddress(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_MFR_ADDRESS, i2c_num);
}

uint dps::ReadMfrSpecialId(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_MFR_SPECIAL_ID, i2c_num);
}

uint dps::ReadPmbusRev(DPS_GROUP_SINGLE sel, uint i2c_num)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandR(I2C_CMD_PMBUS_REV, i2c_num);
}

bool dps::ReadMfrId(DPS_GROUP_SINGLE sel, uint i2c_num, char *dest)
{
	if (sel == DPS_GROUP_SEL)
	{
		printf("%s : error Dps I2C channel sel, use DPS_CHANNEL_SEL\n", __FUNCTION__);
	...
	return GetLtcCommandA(I2C_CMD_PMBUS_REV, i2c_num, dest);
}

// Default Status Word Check. Include Byte Status.
bool dps::StatusCheck(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusWord status;
	cstring strMsg = "";
	...
	SetLtcPage(ltc, page);
	status.WORD_STATUS = GetLtcStatusWord(DPS_SINGLE_SEL, ltc);
	if (m_nDebug)
...
bool dps::StatusMfrCommon(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusMfrCommon status;

	SetLtcPage(ltc, page);
	status.MFR_COMMON = GetLtcStatusWord(DPS_SINGLE_SEL, ltc);
...
bool dps::StatusMfrPads(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusMfrPads status;

	SetLtcPage(ltc, page);
	status.MFR_PADS = GetLtcStatusWord(DPS_SINGLE_SEL, ltc);
...
// Default Status Word Check. Include Byte Status.
bool dps::StatusByte(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusByte status;

	SetLtcPage(ltc, page);
	status.BYTE_STATUS = GetLtcStatusWord(DPS_SINGLE_SEL, ltc);
...
bool dps::StatusInput(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusInput status;
	SetLtcPage(ltc, page);

	status.INPUT_STATUS = GetLtcStatusInput(DPS_SINGLE_SEL, ltc);
	printf("readvin %5.3f\n", ReadVin(DPS_SINGLE_SEL, ltc));
...
bool dps::StatusVout(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusVout status;
	SetLtcPage(ltc, page);

	status.VOUT_STATUS = GetLtcStatusVout(DPS_SINGLE_SEL, ltc);
...
bool dps::StatusIout(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusIout status;
	SetLtcPage(ltc, page);

	status.IOUT_STATUS = GetLtcStatusVout(DPS_SINGLE_SEL, ltc);
...
bool dps::StatusTemperature(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusTemperature status;
	SetLtcPage(ltc, page);

	status.TEMP_STATUS = GetLtcStatusVout(DPS_SINGLE_SEL, ltc);
...
bool dps::StatusCml(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusCml status;
	SetLtcPage(ltc, page);

	status.CML_STATUS = GetLtcStatusVout(DPS_SINGLE_SEL, ltc);
...
bool dps::StatusMfrSpecific(uint ltc, uint page, cstring &msg)
{
	bool bOk = true;
	stStatusMfrSpecific status;
	SetLtcPage(ltc, page);

	status.MFR_SPECFIC_STATUS = GetLtcStatusMfrSpecific(DPS_SINGLE_SEL, ltc);
...
uint dps::DumpCommand(uint ltc, uint page, DPS_PMBUS_COMMAND cmd)
{
	...
	if (GetDpsCommandInfo(cmd, nFmt, nType, nDefault, sCmd))
	{
		...
		{
			rData = GetLtcCommandR(cmd, ltc);
			printf("\t[CH%02d] %-35s 0x%02X : %7.3f\n", ltc, sCmd.c_str(), (int)cmd, GetLtcCommandL(cmd, ltc));
		}
		else if (nFmt == FORMAT_ASC)
		{
			memset(szId, 0x0, sizeof(szId));
			GetLtcCommandA(cmd, ltc, szId);
...
bool dps::ltcDumpCh(uint ltc, uint page)
{
	int nFmt = 0x0;
	int nType = 0x0;
	char szId[32] = {
	...
	for (it = m_mapCmdLtc.begin(); it != m_mapCmdLtc.end(); it++)
	{
		...
		if ((nFmt == FORMAT_L11) || (nFmt == FORMAT_L16))
		{
			...
			printf("\t%-35s 0x%02X : %7.3f\n",
						 sCmd.c_str(), (int)cmd, GetLtcCommandL(cmd, ltc));
		}
		else if (nFmt == FORMAT_ASC)
		{
			memset(szId, 0x0, sizeof(szId));
		...
		{
			printf("\t%-35s 0x%02X :  0x%04X\n", sCmd.c_str(), (int)cmd, GetLtcCommandR(cmd, ltc));
		}
	}
...
bool dps::AllChannelInit()
{
	...
	uint16_t uData = 0;
	int type;//, value;
	vector<int> master = {0, 2, 4, 5};
	// vector<int> master = {0, 2};

	SetLtcPage(0, LTC_PAGE_ALL);
	SetLtcPage(2, LTC_PAGE_ALL);
	SetLtcPage(4, LTC_PAGE_ALL);
	SetLtcPage(5, LTC_PAGE_ALL);
	//I2cSendByte(i2c_num, I2C_CMD_CLEAR_FAULTS);
	I2cSendByteAll(I2C_CMD_CLEAR_FAULTS);

	for (unsigned int num = 0; num < master.size(); num++)
	{
		int i2c_num = master[num];
		//SetLtcPage(i2c_num, LTC_PAGE_ALL);
		//I2cSendByte(i2c_num, I2C_CMD_CLEAR_FAULTS);
		for (unsigned int idx = 0; idx < vecInitCmd.size(); idx++)
		{
			...
			I2cWrite(i2c_num, cmd, uData, (LTC_PMBUS_COMMAND_TYPE)type);

			//SetLtcPage(i2c_num, LTC_PAGE0);
			//uData = I2cRead(i2c_num, I2C_CMD_STATUS_CML, LTC_PMBUS_COMMAND_TYPE::TYPE_R_BYTE);
			//if(uData) {
			//	diag_msg(1, "0 CML FAULT 0x%X ; %s(%d) ; TYPE %d\n", uData, sCmd.c_str(), cmd, type);
			//}
			//SetLtcPage(i2c_num, LTC_PAGE1);
			//uData = I2cRead(i2c_num, I2C_CMD_STATUS_CML, LTC_PMBUS_COMMAND_TYPE::TYPE_R_BYTE);
			//if(uData) {
			//	diag_msg(1, "1 CML FAULT 0x%X ; %s(%d) ; TYPE %d\n", uData, sCmd.c_str(), cmd, type);
			//}
		}
		for (unsigned int idx = 0; idx < vecInitCmd2.size(); idx++)
		{
			...
			sCmd = it->second.Command;
			type = it->second.Type;
			//if(cmd == I2C_CMD_MFR_PWM_COMP && i2c_num > 3) uData = 0x19; // 60A 설정
			// printf("==== %d %s(0x%X) ====\n", i2c_num, sCmd.c_str(), cmd);
			I2cWrite(i2c_num, cmd, uData, (LTC_PMBUS_COMMAND_TYPE)type);
		...
		//SetLtcPage(i2c_num, LTC_PAGE_ALL);
		//I2cSendByte(i2c_num, I2C_CMD_CLEAR_FAULTS);
//		SetLtcClearFaults(master[num]);
//		SetLtcMfrClearPeaks(master[num]);
		Sleep(0.001);
	}
...
bool dps::SetVout(int ch, float fVolt, float under, float over, float limit) /// 0(0,1), 1(2,3), 2(4), 3(5)
{
	if (fVolt > 5.5) {
		printf("%s : error Dps Out Voltage MAX is 5.5, but setting is %f\n", __FUNCTION__, fVolt);
		return false;
	}

	if (ch >= MAX_DPS_USER_NUM) {
		printf("%s : error Dps channel number is invalid, but setting is %d\n", __FUNCTION__, ch);
		return false;
	}
	...
	switch (ch)
	{
		case 0: i2c_num = 0; break;
		case 1:	i2c_num = 2; break;
		case 2:	i2c_num = 4; break;
		case 3:	i2c_num = 5; break;
	}
	...
	if (!SetLtcPage(i2c_num, LTC_PAGE_ALL))
		printf("==== Error(ch%d) : I2C_CMD_PAGE(0x%X) ====\n", i2c_num, I2C_CMD_PAGE);
	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, i2c_num, over))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", i2c_num, I2C_CMD_VOUT_OV_FAULT_LIMIT, over);
	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_WARN_LIMIT, i2c_num, over))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_WARN_LIMIT(0x%X) : Value(%f) ====\n", i2c_num, I2C_CMD_VOUT_OV_FAULT_LIMIT, over);
	if (!SetLtcCommandL(I2C_CMD_VOUT_MARGIN_HIGH, i2c_num, (over + fVolt) / 2.0))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_MARGIN_HIGH(0x%X) :    Value(%f) ====\n", i2c_num, I2C_CMD_VOUT_MARGIN_HIGH, (over + fVolt) / 2.0);
	if (!SetLtcCommandL(I2C_CMD_VOUT_MARGIN_LOW, i2c_num, (under + fVolt) / 2.0))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_MARGIN_LOW(0x%X) :     Value(%f) ====\n", i2c_num, I2C_CMD_VOUT_MARGIN_LOW, (under + fVolt) / 2.0);
	if (!SetLtcCommandL(I2C_CMD_VOUT_UV_FAULT_LIMIT, i2c_num, under))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_UV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", i2c_num, I2C_CMD_VOUT_UV_FAULT_LIMIT, under);
	if (!SetLtcCommandL(I2C_CMD_VOUT_UV_WARN_LIMIT, i2c_num, under))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_UV_WARN_LIMIT(0x%X) : Value(%f) ====\n", i2c_num, I2C_CMD_VOUT_UV_FAULT_LIMIT, under);
	if (!SetLtcCommandL(I2C_CMD_IOUT_OC_WARN_LIMIT, i2c_num, flimit/2))
		printf("==== Error(ch%d) : I2C_CMD_IOUT_OC_FAULT_LIMIT(0x%X) : Value(%f) ====\n", i2c_num, I2C_CMD_IOUT_OC_WARN_LIMIT, flimit);
	if (!SetLtcCommandL(I2C_CMD_VOUT_COMMAND, i2c_num, fVolt))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_COMMAND(0x%X) :        Value(%f) ====\n", i2c_num, I2C_CMD_VOUT_COMMAND, fVolt);

	return true;
}

bool dps::SetHVout(float fVolt)
{
	//if (fVolt > 15.0) {
	//	printf("%s : error VS3 Out Voltage MAX is 15.0, but setting is %f\n", __FUNCTION__, fVolt);
	//	return false;
	//}
	...
	gIO.set_hv_dac(2, fVolt);
}

bool dps::SetHVoutZero()
{
	gIO.set_hv_dac(2, 0.0);
}

bool dps::ReadChVout(int ch, float &VMin, float &VMax)
{
	int i2c_num = 0;
	switch (ch)
	{
	...
	SetLtcPage(i2c_num, LTC_PAGE::LTC_PAGE0);
	fValue = ReadVout(DPS_GROUP_SINGLE::DPS_SINGLE_SEL, i2c_num);
	VMax = VMin = fValue;

	SetLtcPage(i2c_num, LTC_PAGE::LTC_PAGE1);
	fValue = ReadVout(DPS_GROUP_SINGLE::DPS_SINGLE_SEL, i2c_num);
...
float dps::ReadChIout(int ch, bool bCalApply) 
{
	int i2c_num = 0;
	switch (ch)
	{
	...
	SetLtcPage(i2c_num, LTC_PAGE::LTC_PAGE0);
	fValue += ReadIout(DPS_GROUP_SINGLE::DPS_SINGLE_SEL, i2c_num);

	SetLtcPage(i2c_num, LTC_PAGE::LTC_PAGE1);
	fValue += ReadIout(DPS_GROUP_SINGLE::DPS_SINGLE_SEL, i2c_num);

	if(bCalApply) {
		return ((fValue * gOPER.m_DPSIMCal[ch].gain) + gOPER.m_DPSIMCal[ch].offset);
	}
	else
		return fValue;
}

uint dps::ReadAlertCtrl()
{
	return gIO.dps_read_alert();
}

float dps::ReadCvmVout()
{
	short code;
	gIO.cvm_read_voltage(code);
	float volt = 0.00125 * code;
	return volt;
}

float dps::ReadCvmIout()
{
	short code;
	float gain = 1.27;
	// 임시 gain 적용 추 후 calibration 가능해지면 cal 에서 적용 예정
	gIO.cvm_read_current(code);
	float current = CVM_CUR_LSB * code * gain;
...
{
	// Over Voltage
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 0, m_OVFVal[0]))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 0, I2C_CMD_VOUT_OV_FAULT_LIMIT, m_OVFVal[0]);
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 1, m_OVFVal[1]))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 1, I2C_CMD_VOUT_OV_FAULT_LIMIT, m_OVFVal[1]);

	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 2, m_OVFVal[2]))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 2, I2C_CMD_VOUT_OV_FAULT_LIMIT, m_OVFVal[2]);
	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 3, m_OVFVal[3]))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 3, I2C_CMD_VOUT_OV_FAULT_LIMIT, m_OVFVal[3]);

  // Under Voltage
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 0, m_UVFVal[0]))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 0, I2C_CMD_VOUT_OV_FAULT_LIMIT, m_UVFVal[0]);
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 1, m_UVFVal[1]))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 1, I2C_CMD_VOUT_OV_FAULT_LIMIT, m_UVFVal[1]);

	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 2, m_UVFVal[2]))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 2, I2C_CMD_VOUT_OV_FAULT_LIMIT, m_UVFVal[2]);
	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 3, m_UVFVal[3]))
...
{
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 0, 5.5))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 0, I2C_CMD_VOUT_OV_FAULT_LIMIT, 5.5);
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 1, 5.5))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 1, I2C_CMD_VOUT_OV_FAULT_LIMIT, 5.5);

	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 2, 5.5))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 2, I2C_CMD_VOUT_OV_FAULT_LIMIT, 5.5);
	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 3, 5.5))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 3, I2C_CMD_VOUT_OV_FAULT_LIMIT, 5.5);

	// Under Voltage
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 0, 0.0))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 0, I2C_CMD_VOUT_OV_FAULT_LIMIT, 0.0);
	if (!SetLtcCommandLGlobal(I2C_CMD_VOUT_OV_FAULT_LIMIT, 1, 0.0))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 1, I2C_CMD_VOUT_OV_FAULT_LIMIT, 0.0);

	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 2, 0.0))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 2, I2C_CMD_VOUT_OV_FAULT_LIMIT, 0.0);
	if (!SetLtcCommandL(I2C_CMD_VOUT_OV_FAULT_LIMIT, 3, 0.0))
		printf("==== Error(ch%d) : I2C_CMD_VOUT_OV_FAULT_LIMIT(0x%X) : Value(%f) ====\n", 3, I2C_CMD_VOUT_OV_FAULT_LIMIT, 0.0);

	return true;
}

bool dps::SetLtcUserData0(uint i2c_num, word data)
{
	return SetLtcCommandR(I2C_CMD_USER_DATA_00, i2c_num, data);
}

uint dps::GetLtcUserData0(uint i2c_num)
{
	return GetLtcCommandR(I2C_CMD_USER_DATA_00, i2c_num);
}

bool dps::SetLtcUserData2(uint i2c_num, word data)
{
	return SetLtcCommandR(I2C_CMD_USER_DATA_02, i2c_num, data);
}

uint dps::GetLtcUserData2(uint i2c_num)
{
	return GetLtcCommandR(I2C_CMD_USER_DATA_02, i2c_num);
}

bool dps::SetLtcUserData4(uint i2c_num, word data)
{
	return SetLtcCommandR(I2C_CMD_USER_DATA_04, i2c_num, data);
}

uint dps::GetLtcUserData4(uint i2c_num)
{
	return GetLtcCommandR(I2C_CMD_USER_DATA_04, i2c_num);
}
