...
int star_session::starRead()
{
	...
	case PKT_STATE_HEADER:
		...
		if ((unsigned)pkt_header.magic != PKT_MAGIC) {
			printf("star_session::%s: not matched magic\n", __FUNCTION__);
			soc.rd.reset();
			return S_ERR_QUEUED;
		}
	...
	case PKT_STATE_DATA:
		...
		if (sz != 0) {
			...
			if (err != 0) {
				printf("star_session::%s: alloc() error. sz=%d\n", __FUNCTION__, org_sz + 1);
				soc.rd.reset();
				nPacketState = PKT_STATE_HEADER;
	...
	default:
		printf("star_session::%s: abnormal process", __FUNCTION__);
		nPacketState = PKT_STATE_HEADER;
		soc.rd.reset();
		break;
...
int star_session::starResult(int seq, int status, const char* rstr, int pktType)
{
	...
	if (resp == S_ERR_GENERAL) {
		printf("star_session::%s: write fail\n", __FUNCTION__);
		end();
	}
	return resp;
}
...
bool star_session::starGetResponse(const char* cmd, bool& cmd_return)
{
	...
	if (ret == S_ERR_GENERAL) {
		...
		printf("star_session::%s: close connection in client\n", __FUNCTION__);
		return false;
	}
	if (ret == S_ERR_QUEUED)
		return false;
...
int star_session::starRecvFile()
{
	...
	if (ofs != -1) {
		...
		printf("star_session::%s: FecvFile Complete - %s\n", __FUNCTION__, sRecvFilename);
		return 1;
	}
...
bool star_session::starSendFile(const char* path, int mode, const char* source, const char* target, int client)
{
	...
	if ((f = fopen(source, "rb")) == NULL) {
		printf("star_session::%s: File Open Error \'%s\'", __FUNCTION__, fn);
		return false;
	}

	if (fseek(f, 0L, SEEK_END) != 0) {
		printf("star_session::%s: FSeek Error \'%s\' ", __FUNCTION__, fn);
		fclose(f);
		return false;
	}
	...
	sz = ftell(f);
	fseek(f, 0L, SEEK_SET);
	printf("star_session::%s:send file sz %ld\n", __FUNCTION__, sz);
	int compressed;
	...
	while (sz > 0) {
		...
		if (fread(rbuf, rsz, 1, f) != 1) {
			printf("star_session::%s: FRead Error \'%s\' ", __FUNCTION__, fn);
			end();
			ret = false;
			goto errexit;
		}
		...
		Ack_offset = ofs;
		//printf("star_session::%s: path=%s, dir=%s, name=%s, sz=%d\n", __FUNCTION__, path, dir, fn, rsz);
		if (client == -1) {
			l = sprintf(sbuf, "<star-p path=\"%s\"><send-file dir=\"%s\" name=\"%s\" ofs=\"%ld\" size=\"%ld\" compress=\"%s\" org_sz=\"%ld\">", 
				path, dir, fn, ofs, send_sz, compressed == 1 ? "yes" : "no", rsz);
		} else {
			l = sprintf(sbuf, "<star-p client=\"%d\" path=\"%s\"><send-file dir=\"%s\" name=\"%s\" ofs=\"%ld\" size=\"%ld\" compress=\"%s\" org_sz=\"%ld\">", 
				client, path, dir, fn, ofs, send_sz, compressed == 1 ? "yes" : "no", rsz);
		}	
		p = sbuf + l;
		memcpy(p, pp, send_sz);
		...
		if (starWrite(mode, l, sbuf) == S_ERR_GENERAL  || soc.bufFlush() == S_ERR_GENERAL) {
			printf("star_session::%s: failed on send\n", __FUNCTION__);
			end();
			ret = false;
			goto errexit;
		}
		...
		if ((Ack_offset % HANDSHAKE_SZ) == 0 && Ack_offset != 0)
		{
			...
			if (!starWaitResponse("SEND-FILE", 120)) {
				printf("star_session::%s: wait response Fail\n", __FUNCTION__);
				end();
				ret = false;
				bWaitResponse = false;
				goto errexit;
		...
		if (starGetResponse("SEND-FILE", cmdret)) {
			...
			if (!cmdret) {
				printf("star_session::%s: failed: ret=%s\n", __FUNCTION__, packet.buf);
				ret = false;
				goto errexit;
			}
		}
	...
	mode = PKT_TYPE_REQDONE;// src_mode & (~PKT_COMPRESSED);
	...
	if (starWrite(mode, l, buf) == S_ERR_GENERAL || soc.bufFlush() == S_ERR_GENERAL) {
		printf("star_session::%s: last write Fail\n", __FUNCTION__);
		end();
		ret = false;
		goto errexit;
	}
