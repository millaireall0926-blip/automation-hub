...
namespace HTCRack_CXL.Net
{
    ...
    public class HtcFilter : FixedHeaderReceiveFilter<HtcPacket>
    {
        ...
        protected override int GetBodyLengthFromHeader(IBufferStream bufferStream, int length)
        {
            ...
            {
#if TEST_MODE
                Console.WriteLine("catch {1}: {0}", ex.Message, System.Reflection.MethodBase.GetCurrentMethod().Name);
#endif
    ...
    public class HtcClient : EasyClient
    {
        ...
        //public ConcurrentQueue<byte[]> _sendQueue = new ConcurrentQueue<byte[]>();
        ...
        public void pushSendQue(REG_TYPE type, HtcMessage msg)
        {
            ...
            {
                if (Global.gMainVM.ShowEngineer == Visibility.Visible && Global.gEngineerVM.CurRack == _index)
                {
                    ...
                    Global.gEngineerVM.pushCmdLog(new CommandLog(_index, CMD_TYPE.SEND, sb.ToString()));
                }
            }
            lock (_lock)
            {
        ...
        public byte[] popSendQue()
        {
            ...
            //_sendQueue.TryDequeue(out data);
        ...
        public void processPacket(HtcPacket pack)
        {
            ...
            // UI 스레드에서 실행하도록 작업을 전달
            Application.Current.Dispatcher.Invoke(() =>
            {
                if (funcRecv != null) funcRecv(msg);
            });
            //if (funcRecv != null)
            //    funcRecv(msg);
