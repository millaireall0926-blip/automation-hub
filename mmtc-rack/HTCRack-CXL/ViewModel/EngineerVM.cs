...
namespace HTCRack_CXL.ViewModel
{
    ...
    public class EngineerVM : BaseVM
    {
        ...
        #region MOTOR
        public bool ExpandMOTOR { get; set; } = false;
        public ObservableCollection<RegModel> MOTORRegModels { get; set; } = new ObservableCollection<RegModel>();
        ...
        public EngineerVM()
        {
            ...
            RegMap.HSENSOR_Hum = MOTORRegModels.Count;
            adr = 0;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "%", "Get H.Sensor Humidity1", len: 2)); adr++;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "%", "Get H.Sensor Humidity2", len: 2)); adr++;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "%", "Get H.Sensor Humidity3", len: 2)); adr++;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "%", "Get H.Sensor Humidity4", len: 2));

            RegMap.HSENSOR_Temp = MOTORRegModels.Count;
            adr = 0;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "°C", "Get H.Sensor Temperature1", len: 2)); adr++;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "°C", "Get H.Sensor Temperature2", len: 2)); adr++;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "°C", "Get H.Sensor Temperature3", len: 2)); adr++;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", adr.ToString(), "0", "°C", "Get H.Sensor Temperature4", len: 2));

            RegMap.HI_Limit = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "-", "Get MOTOR HI Limit Sensor (1-bit Active HIGH)", len: 1));

            RegMap.LO_Limit = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "-", "Get MOTOR Low Limit Sensor (1-bit Active HIGH)", len: 1));

            RegMap.CBOX_Contact = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "0x", "Get Box Contact Sensor (4-bit Active HIGH)", len: 1));

            RegMap.MOTOR_Lock = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "-", "Get Motor Lock (1-bit Active HIGH)", len: 1));

            RegMap.MOTOR_En = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "-", "Get Motor Enable (1-bit Active HIGH)", len: 1));

            RegMap.MOTOR_Dir = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "-", "Get Motor Direction (0 : DN, 1 : UP", len: 1));

            RegMap.MOTOR_Run = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "-", "Get Motor Run (0 : Stop, 1 : Auto, 2 : Manual", len: 1));

            RegMap.MOTOR_Step = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "0", "-", "Get Motor Step ", len: 4));

            RegMap.MOTOR_Sensor = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "40", "0x", "Set HI/LO LIMIT POLARITY : 0 - default / 1 - Inv", noRead: true, send: true, len: 1));
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "41", "0x", "Set CBOX CONTACT POLARITY : 0 - default / 1 - Inv", noRead: true, send: true, len: 1));

            RegMap.MOTOR_Stat = MOTORRegModels.Count;
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "42", "0x", "Set MOTOR DIRECTION POLARITY : 0 - default / 1 - Inv", noRead: true, send: true, len: 1));
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "43", "-", "Set MOTOR STEPPING FREQ : default - 100,000 for 100us", noRead: true, send: true, len: 4));
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "44", "-", "Set MOTOR STEPPING MAX COUNT : default - 55000", noRead: true, send: true, len: 4));
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "50", "0x", "Set MOTOR AUTO CTRL : 0 - STOP(AT & MAN), 1 - Up, 2 - Down", noRead: true, send: true, len: 1));
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "51", "-", "Set MOTOR MANUAL UP : 0 - default AUTO / 1 ~ for Manual", noRead: true, send: true, len: 4));
            MOTORRegModels.Add(new RegModel(REG_TYPE.PDO, "ff", "0", "52", "-", "Set MOTOR MANUAL DOWN : 0 - default AUTO / 1 ~ for Manual", noRead: true, send: true, len: 4));
            ...
            foreach(RegModel reg in MOTORRegModels)
            {
                if (reg.FUNC == "43" || reg.FUNC == "44" || reg.FUNC == "51" || reg.FUNC == "52")
                    reg.funcWriteToHex += doHextoDouble;
            }
        }
        ...
        public string doHextoDouble(double vlaue)
        {
            int cnv = (int)vlaue;
            return cnv.ToString("X");
        }
        ...
        public void setRack()
        {
            ...
            */
            ...
            index = 0;
            foreach(RegModel rm in rack.MOTORRegs)
            {
                MOTORRegModels[index].Read = rm.Read;
                MOTORRegModels[index].ReadValue = rm.ReadValue;
                index++;
            }
