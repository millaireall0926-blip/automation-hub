...
//        public byte SSRCtrl { get; set; } = 0;// SSR is a synthetic bit, so save and use it.
        ...
        public double[] _prevDimmEqualCounts = new double[64];
...
//        Random rand = new Random();
        ...
        public void startTest()
        {
            ...
            Array.Clear(_prevDimmEqualCounts, 0, _prevDimmEqualCounts.Length);
        ...
        public void processTest()
        {
            if (RackStat == RACK_STAT.RUN)
            {
                ...
                {
                    box.processBoxDimm();
                    if (box.InsideBoxTemp < Global.gConfigVM.MinTempPV)
                    {
                        showAlram(ALRAM_TYPE.ERROR, $"[Prog][{Name}][{box.Name}]",$" PV Temp Under {Global.gConfigVM.MinTempPV}°C");
                        RackStat = RACK_STAT.ERROR;
                    }
