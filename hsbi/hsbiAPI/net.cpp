...
int star_client::send(char* str, int toDaemon)
{
	...
	//	printf("%s\n", buf.c_str());
	int mode = STAR_TO_OS;
	if (toDaemon)mode = STAR_TO_DAEMON;
		...
		for (e = gLIB.xml_child(e); e; e = gLIB.xml_next(e))
		{
			...
			if (name == "pgm")
			{
				...
				if (sub == "temp")
				{
					...
					gOPR.m_nChamSt = atoi(gLIB.xml_att_find(ht, "st"));
					gOPR.m_fReadTemp = atof(gLIB.xml_att_find(ht, "pv"));
...
int net::shm_init()
{
	...
	m_pShmDef->bContact = (gCOM.m_nTestMode == 1) ? true : false;
	if (m_pShmDef->bContact) sprintf(m_pShmDef->sLotID, gCOM.m_sLot);

	sprintf(m_pShmDef->sPgmPath, "/star/pgm");
	sprintf(m_pShmDef->sPgmName, gCOM.m_sPgm);
	sprintf(gCOM.m_sBid, "0x%X", m_pShmDef->nBid);
...
int net::sendTemp(int  mode)
{
	...
	cstring str;
	str.Format("<temp cmd=\"%s\" sv=\"%.1f\" />", tcmd.c_str(), gOPR.m_fSetTemp);
...
int net::send_binary(cstring cmd, byte* bp, int sz)
{
	if (m_clClient.m_hClient == 0)return 0;
	if (m_clClient.m_nCon == 0)return 0;
		
	cstring strST;	
	strST.Format("<star-p path=\"%s\"><binary cmd=\"%s\" sz=\"%d\" >",
		m_clClient.star_path, cmd.c_str(), sz );

	cstring strED;
	strED.Format("</binary></star-p>");

	int stsz = strST.GetLength();
	int mdsz = sz;
	int edsz = strED.GetLength();
	int sumsz = stsz + mdsz + edsz;
	
	byte* sendBuf = new byte[sumsz];
	byte* p = (byte*)strST.c_str();
	memcpy(sendBuf, p, stsz);	
	
	memcpy(&sendBuf[stsz], bp, mdsz);
	 
	p = (byte*)strED.c_str();
	memcpy(&sendBuf[stsz+mdsz], p, edsz);
		    
    return gLIB.star_send(m_clClient.m_hClient, STAR_TO_OS | PKT_TYPE_SENDBIN, reinterpret_cast<const char*>(sendBuf), sumsz);
...
int net::sendPowerSet()
{
	printf("\n!!! Send Power SET~~~ \n");

	cstring str;
	str.Format("<power-hsbi><set>");	
	
	// DPS Setting
	str.AppendFormat("<dps>");	
	for (int i = 0; i < MAX_DPS_CH; i++) {
		str.AppendFormat("%.2f,", gOPR.m_PMonArr[i].sv);
		str.AppendFormat("%.2f,", gOPR.m_PMonArr[i].oc);
	}
	str.AppendFormat("</dps>");

	// HDPS
	str.AppendFormat("<hdps>");
	for (int i = 0; i < 2; i++) {
		str.AppendFormat("%.2f,", gOPR.m_PMonArr[MAX_DPS_CH +i].sv);		
	}
	str.AppendFormat("</hdps>");

	str.AppendFormat("<pe>"); 
	str.AppendFormat(" % .2f,", gOPR.m_IOVIH);
	str.AppendFormat(" % .2f,", gOPR.m_DRVIH);
	str.AppendFormat(" % .2f,", gOPR.m_IOVTT);
	str.AppendFormat(" % .2f,", gOPR.m_DRVTT);

	str.AppendFormat(" % .2f,", gOPR.m_DutIOVTT);
	str.AppendFormat(" % .2f,", gOPR.m_DutDRVTT);	
	str.AppendFormat(" % .2f,", gOPR.m_DutVREF);

	str.AppendFormat(" % .2f,", gOPR.m_VTH);
	str.AppendFormat("</pe>");


	str.AppendFormat("</set></power-hsbi>");
...
int net::sendPower()
{
	...
	for (int i = 0; i < MAX_DPS_CH; i++)
	{
		...
		if (m_nSendPower == SEND_PWR_READ) {
			rv = gOPR.m_PMonArr[i].rv;
			rc = gOPR.m_PMonArr[i].rc;
		}

		str.AppendFormat("%.2f,", rv);
		str.AppendFormat("%.2f,", rc);
	}
	str.AppendFormat("</dps>");
	str.AppendFormat("</read></power-hsbi>");
	send_result("pgm", 1, str);
...
{
	if (gCOM.m_nTestMode == TEST_MODE_DIAG) return 1; // Diag Test Mode Not Setting Normal Seq
...
{
	cstring str;
	str.Format("<start-test id=\"star\" con=\"%d\"/>", (gCOM.m_nTestMode == TEST_MODE_CONTACT));
	send_result("pgm", 1, str);
	return 1;
...
int net::sendStartStep()
{
	...
	str.Format("<start-step id=\"%s\" con=\"%d\" />",
		gRES.m_sResCurStep, (gCOM.m_nTestMode == TEST_MODE_CONTACT));
