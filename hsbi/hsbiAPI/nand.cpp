...
void nand::setLimit(int col, int page, int block)
{
	...
	uint adr = (1 << 9) | 35;
	gIO.writePG(PCI1, PG_ALL, adr, col, 2);
	gIO.writePG(PCI2, PG_ALL, adr, col, 2);
	adr = (1 << 9) | 37;
	gIO.writePG(PCI1, PG_ALL, adr, page, 2);
	gIO.writePG(PCI2, PG_ALL, adr, page, 2);
}
...
void nand::readInfo()		// Fail Info Block Read 
{
	...
	{
		int ret = gOPR.waitTest(SLEEP_10US);
...
//	memcpy(fmArr[1], fmArr[0], FM_MEM_SZ * FM_DEPTH);
	...
	uint adr = 4 << 17;
	gIO.writePGData(adr, &fmArr[0][0][0], FM_MEM_SZ * FM_DEPTH);
...
//	printf("\n");
	...
	for (int i = 0; i < 8; i++)
	{		
		gIO.writePGData(PCI1, PG_ALL, (1 << 20) | (2 << 18) | ((i % 8) << 15), pData, 3096 * 8);
		gIO.writePGData(PCI2, PG_ALL, (1 << 20) | (2 << 18) | ((i % 8) << 15), pData, 3096 * 8);
	}
...
void nand::testFailCountDetail()
{
	...
	for (int i = 0; i < scansz; )
	{
		int ret = gOPR.waitTest(SLEEP_1US);
		printf("OUT CMD % x\n", ret);
		if (ret == SYNC_END) break;
...
{
	int rCnt = gFM.readFmData(0);
	rCnt += gFM.readFmData(1);
	rCnt += gFM.readFmData(2);
	rCnt += gFM.readFmData(3);
