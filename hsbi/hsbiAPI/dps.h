...
// typedef std::vector<stPSeq> vecPSeq;
...
public:
	float GetLtcCommandL(PCI_SEL pci, DPS_PMBUS_COMMAND, int);				 // L11, L16
	uint GetLtcCommandR(PCI_SEL pci, DPS_PMBUS_COMMAND, int);				 // REG, I16
	bool GetLtcCommandA(PCI_SEL pci, DPS_PMBUS_COMMAND, int, char *); // REG, I16
	// bool  SetLtcCommandL( DPS_PMBUS_COMMAND, DPS_GROUP_SINGLE, int, float );		//L11, L16
	// bool  SetLtcCommandR( DPS_PMBUS_COMMAND, DPS_GROUP_SINGLE, int, uint  );		//REG, I16
	bool SetLtcCommandL(PCI_SEL pci, DPS_PMBUS_COMMAND, int, float);				// L11, L16
	bool SetLtcCommandLGlobal(PCI_SEL pci, DPS_PMBUS_COMMAND, int, float); // L11, L16
	bool SetLtcCommandR(PCI_SEL pci, DPS_PMBUS_COMMAND, int, uint);				// REG, I16

public:
	bool SetGangMode(int idx, bool bOn);
	bool I2cWaitEos(PCI_SEL pci);
	// bool I2cWrite   ( DPS_GROUP_SINGLE, int, int, int, LTC_PMBUS_COMMAND_TYPE );
	bool I2cWrite(PCI_SEL pci, int, int, int, LTC_PMBUS_COMMAND_TYPE);
	bool I2cWriteGlobal(PCI_SEL pci, int, int, int, LTC_PMBUS_COMMAND_TYPE);
	uint I2cRead(PCI_SEL pci, int, int, LTC_PMBUS_COMMAND_TYPE);
	bool I2cReadA(PCI_SEL pci, int, int, uint, char *);
	...
	uint I2cSendByte(PCI_SEL pci, int, int);
	uint I2cSendByteAll(int);

public:
	bool StatusCheck(PCI_SEL, uint, uint, cstring &); // Default Status Word.
	bool StatusByte(PCI_SEL, uint, uint, cstring &);
	bool StatusInput(PCI_SEL, uint, uint, cstring &);
	bool StatusMfrSpecific(PCI_SEL, uint, uint, cstring &);
	bool StatusVout(PCI_SEL, uint, uint, cstring &);
	bool StatusIout(PCI_SEL, uint, uint, cstring &);
	bool StatusTemperature(PCI_SEL, uint, uint, cstring &);
	bool StatusCml(PCI_SEL, uint, uint, cstring &);
	bool StatusMfrCommon(PCI_SEL, uint, uint, cstring &);
	bool StatusMfrPads(PCI_SEL, uint, uint, cstring &);

	bool ltcDumpCh(PCI_SEL, uint, uint);
	uint DumpCommand(PCI_SEL, uint, uint, DPS_PMBUS_COMMAND);

public:
	bool SetLtcPage(PCI_SEL, uint, uint);
	bool SetLtcPageAll(uint);
	bool SetLtcOperation(PCI_SEL, uint, LTC_PMBUS_OPPERATION);
	bool SetLtcOnOffConfig(PCI_SEL, uint, LTC_PMBUS_ON_OFF_CONFIG);
	bool SetLtcClearFaults(PCI_SEL, uint, uint d = 0x0);
	bool SetLtcMfrClearPeaks(PCI_SEL, uint, uint d = 0x0);
	bool SetLtcMfrReset(PCI_SEL, uint, uint d = 0x0);
	bool SetLtcStoreUserAll(PCI_SEL, uint, uint d = 0x0);
	bool SetLtcRestoreUserAll(PCI_SEL, uint, uint d = 0x0);

	bool setLtcMFRFaultPropagate(PCI_SEL, uint, uint);

	bool SetLtcFreqSwitch(PCI_SEL, uint, uint);

	bool SetLtcVoutCommand(PCI_SEL, uint, float);
	bool SetLtcVoutMax(PCI_SEL, uint, float);
	bool SetLtcVoutMarginHigh(PCI_SEL, uint, float);
	bool SetLtcVoutMarginLow(PCI_SEL, uint, float);
	bool SetLtcVoutTransRate(PCI_SEL, uint, float);
	bool SetLtcVoutOvFaultLimit(PCI_SEL, uint, float);
	bool SetLtcVoutOvFaultResponse(PCI_SEL, uint, uint);
	bool SetLtcVoutOvWarnLimit(PCI_SEL, uint, float);
	bool SetLtcVoutUvWarnLimit(PCI_SEL, uint, float);
	bool SetLtcVoutUvFaultLimit(PCI_SEL, uint, float);
	bool SetLtcVoutUvFaultResponse(PCI_SEL, uint, uint);

	bool SetLtcVinOn(PCI_SEL, uint, float);
	bool SetLtcVinOff(PCI_SEL, uint, float); // khz
	bool SetLtcVinOvFaultLimit(PCI_SEL, uint, float);
	bool SetLtcVinOvFaultResponse(PCI_SEL, uint, uint);
	bool SetLtcVinUvWarnLimit(PCI_SEL, uint, float);

	bool SetLtcIinOcWarnLimit(PCI_SEL, uint, float);

	bool SetLtcIOutCalGain(PCI_SEL, uint, float); // mohm
	bool SetLtcIoutOcFaultLimit(PCI_SEL, uint, float);
	bool SetLtcIoutOcFaultResponse(PCI_SEL, uint, uint);
	bool SetLtcIoutOcWarnLimit(PCI_SEL, uint, float);

	bool SetLtcOtFaultLimit(PCI_SEL, uint, float);
	bool SetLtcOtFaultResponse(PCI_SEL, uint, uint);
	bool SetLtcOtWarnLimit(PCI_SEL, uint, float);

	bool SetLtcUtFaultLimit(PCI_SEL, uint, float);
	bool SetLtcUtFaultResponse(PCI_SEL, uint, uint);

	bool SetLtcTonDelay(PCI_SEL, uint, float);					// ms
	bool SetLtcTonRise(PCI_SEL, uint, float);					// ms
	bool SetLtcTonMaxFaultLimit(PCI_SEL, uint, float); // ms
	bool SetLtcToffDelay(PCI_SEL, uint, float);				// ms
	bool SetLtcToffFall(PCI_SEL, uint, float);					// ms
	bool SetLtcToffMaxWarnLimit(PCI_SEL, uint, float); // ms

	bool setMfrIinCalGain(PCI_SEL, uint, float);

	bool SetLtcMfrRVin(PCI_SEL, uint, float);		// mohm
	bool SetLtcMfrPwmComp(PCI_SEL, uint, uint);	// REG
	bool SetLtcMfrPwmMode(PCI_SEL, uint, uint);	// REG
	bool SetLtcRailAddress(PCI_SEL, uint, uint); // REG
	bool SetLtcPolyMode(PCI_SEL, uint, uint);

	bool SetLtcUserData0(PCI_SEL, uint, word);
	bool SetLtcUserData2(PCI_SEL, uint, word);
	bool SetLtcUserData4(PCI_SEL, uint, word);

	uint getLtcMfrPwmMode(PCI_SEL, uint); // REG
	uint GetDpsVoutMode(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusByte(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusWord(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusVout(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusIout(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusInput(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusTemperature(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusCml(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcStatusMfrSpecific(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint GetLtcMfrCommon(PCI_SEL, DPS_GROUP_SINGLE, uint);
	//uint GetLtcMfrPads(DPS_GROUP_SINGLE, uint);
	uint GetLtcMfrPads(PCI_SEL, uint ch);

	uint GetLtcUserData0(PCI_SEL, uint);
	uint GetLtcUserData2(PCI_SEL, uint);
	uint GetLtcUserData4(PCI_SEL, uint);

	float GetMfrVoutMax(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadVin(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadIin(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadVout(PCI_SEL, DPS_GROUP_SINGLE, uint, uint sampleing = 1);
	float ReadIout(PCI_SEL, DPS_GROUP_SINGLE, uint, uint sampleing = 1);
	float ReadTemperature_1(DPS_GROUP_SINGLE, uint);
	float ReadTemperature_2(DPS_GROUP_SINGLE, uint);
	float ReadMfrTemperature_1_Peak(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadMfrTemperature_2_Peak(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadFrequency(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadPout(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadPin(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint ReadPmbusRev(PCI_SEL, DPS_GROUP_SINGLE, uint);
	bool ReadMfrId(PCI_SEL, DPS_GROUP_SINGLE, uint, char *);
	bool ReadMfrModel(DPS_GROUP_SINGLE, uint, char *);
	float ReadMfrVoutPeak(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadMfrIoutPeak(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadMfrVinPeak(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadMfrIinPeak(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadMfrReadIChip(PCI_SEL, DPS_GROUP_SINGLE, uint);
	float ReadMfrIinCalGain(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint ReadMfrAddress(PCI_SEL, DPS_GROUP_SINGLE, uint);
	uint ReadMfrSpecialId(PCI_SEL, DPS_GROUP_SINGLE, uint);

public:
	bool AllChannelInit();
	bool SetVout(int ch, float fVolt, float under, float over, float limit, bool bCalApply = true);
	bool SetHVout(int ch, float fVolt);
	...
	uint ReadAlertCtrl(PCI_SEL pci);
	float ReadInaVout(int ch);
	float ReadInaIout(int ch);
	void OnSeq(uint8_t idx, uint16_t dps_on_bit, float delay);
	void OffSeq(uint8_t idx, uint16_t dps_on_bit, float delay);

	bool CommitVoutFaultLimit();
	bool ReleaseVoutFaultLimit();

	bool ReadAllreg(int nDps);
protected:
	// vecPSeq m_OnSeq;
	// vecPSeq m_OffSeq;
	std::vector<float> m_VOutVal;
	std::vector<float> m_UVFVal;
	std::vector<float> m_OVFVal;
public:
	std::vector<stCalFactor> m_VSCal;
	std::vector<stCalFactor> m_VMCal;
	std::vector<stCalFactor> m_IMCal;
};
