...
int cmd_o(int argc, char** argv)
{
	...
	gIO.writePG(PCI1, PG_ALL, nArgs[0] , nArgs[1], 1);
    gIO.writePG(PCI2, PG_ALL, nArgs[0] , nArgs[1], 1);
	printf("[o] : %06x -> %02x\n", nArgs[0] , nArgs[1]);
	delete[] nArgs;
	return 1;
}
...
int cmd_i(int argc, char** argv)
{
	...
	uint rd = gIO.readPG(PCI1, PG_0, adr , 1);
	printf("PCI0 [i] : %06x <- %02x\n", adr , rd);

    rd = gIO.readPG(PCI2, PG_0, adr , 1);
	printf("PCI1 [i] : %06x <- %02x\n", adr , rd);
...
int cmd_ia(int argc, char** argv)
{
	...
	gIO.readPGData(PCI1, 0x1, nArgs[0] , pData, nArgs[1]);
    ...
    gIO.readPGData(PCI2, 0x1, nArgs[0] , pData, nArgs[1]);
	printf("[ia] adr 0x%06x size %d", nArgs[0], nArgs[1]);
	for (int i = 0; i < (int)nArgs[1]; i++)
	{
		if (i % 16 == 0)
...
int cmd_oa(int argc, char** argv)
{
	...
	gIO.writePGData(PCI1, PG_ALL, adr, pData, size);
	printf("PCI0 [oa] adr 0x%06x size %d\n", adr, size);

    gIO.writePGData(PCI2, PG_ALL, adr, pData, size);
	printf("PCI1 [oa] adr 0x%06x size %d\n", adr, size);
...
int cmd_O(int argc, char** argv)
{
	...
	gIO.writePGI(PCI1, nArgs[0], nArgs[1], nArgs[2], 1);
	printf("PCI0 [O]P%01x : %06x -> %02x\n",nArgs[0], nArgs[1], nArgs[2]);

    gIO.writePGI(PCI2, nArgs[0], nArgs[1], nArgs[2], 1);
	printf("PCI1 [O]P%01x : %06x -> %02x\n",nArgs[0], nArgs[1], nArgs[2]);
...
int cmd_I(int argc, char** argv)
{
	...
	uint rd = gIO.readPGI(PCI1, 0x1, nArgs[0], nArgs[1]);
	printf("PCI0 [I]p%01x : %06x <- %02x\n", nArgs[0], nArgs[1], rd);

    rd = gIO.readPGI(PCI2, 0x1, nArgs[0], nArgs[1]);
	printf("PCI1 [I]p%01x : %06x <- %02x\n", nArgs[0], nArgs[1], rd);
...
int cmd_IA(int argc, char** argv)
{
	...
	byte* pData = new byte(nArgs[1]);
	gIO.readPGDataI(PCI1, nArgs[0], nArgs[1], pData, nArgs[2]);
    ...
    gIO.readPGDataI(PCI2, nArgs[0], nArgs[1], pData, nArgs[2]);
	printf("[IA]P%01x : adr 0x%06x size %d", nArgs[0], nArgs[1], nArgs[2]);
	for (int i = 0; i < (int)nArgs[1]; i++)
	{
		if (i % 16 == 0)
...
int cmd_OA(int argc, char** argv)
{
	...
	gIO.writePGDataI(PCI1, pgi, adr, pData, size);
	printf("PCI0 [OS]P%01x: adr 0x%06x size %d\n", pgi, adr, size);

    gIO.writePGDataI(PCI2, pgi, adr, pData, size);
	printf("PCI1 [OS]P%01x: adr 0x%06x size %d\n", pgi, adr, size);
...
int cmd_help(int argc, char** argv)
{
	...
	printf("TG_REG			: 0x%06x\n", 	TG_REG );
	printf("TG_MEM			: 0x%06x\n", 	TG_MEM );
