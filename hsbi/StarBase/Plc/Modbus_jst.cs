...
namespace StarBase.Plc {
    ...
    public class Modbus_jst : Modbus {
        ...
        public Modbus_jst() 
        {
            ...
            enableWatchDocFunc = enableWatchDog;
            toggleWatchDogFunc = toggleWatchDog;
            setWatchDogTimeoutFunc = setWatchDogTimeout;
        }
        ...
        public bool setBuzzer(int buzzer) // 0 = OS, 1 = PLC
{
        ...
        public bool enableWatchDog(bool isEnable)
        {
            string cmd = string.Format("D{0:D5},{D}", PLCMap_jst.PLC_WT_WATCH_ENABLE_W, isEnable? 1:0 );            
            return request(cmd);
        }

        int _watchDogToggle = 0;
        public bool toggleWatchDog()
        {
            if (_watchDogToggle == 0) _watchDogToggle = 1;
            else _watchDogToggle = 0;

            string cmd = string.Format("D{0:D5},{D}", PLCMap_jst.PLC_WT_WATCH_TOGGLE_W, _watchDogToggle);
            return request(cmd);

        }

        public bool setWatchDogTimeout(int sec)
        {
            string cmd = string.Format("D{0:D5},{D}", PLCMap_jst.PLC_WT_WATCH_TIME_W, sec);
            return request(cmd);
        }
