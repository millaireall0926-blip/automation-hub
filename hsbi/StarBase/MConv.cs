...
namespace StarBase
{
    public class MConv
    {
        ...
        public static object GetProperty<T>(T obj, string name)
        {
            PropertyInfo info = typeof(T).GetProperty(name);
            if (info == null) throw new ArgumentNullException($"Object cannot Find {name}"); ;
        ...
        public void setField<T>(string name, object value)
        {
            Type type = typeof(T);
